...
BOOST_FIXTURE_TEST_SUITE(operation_time_tests, clean_database_fixture)
    ...
    BOOST_AUTO_TEST_CASE(comment_payout_dust) {
        try {
            ...
            BOOST_REQUIRE(
                    db.get_dynamic_global_properties().total_reward_fund_steem ==
                    ASSET("0.001 TESTS"));
...
*/
    BOOST_AUTO_TEST_CASE(vesting_withdrawals) {
        try {
            ...
            op.vesting_shares = asset(
                    new_alice.vesting_shares.amount / 2, VESTS_SYMBOL);
            tx.set_expiration(
                    db.head_block_time() + STEEMIT_MAX_TIME_UNTIL_EXPIRATION);
            ...
            auto next_withdrawal = db.head_block_time() +
            ...
            BOOST_REQUIRE((withdraw_rate *
                           gpo.get_vesting_share_price()).amount.value -
            ...
            BOOST_REQUIRE(fill_op.withdrawn.amount.value ==
                          withdraw_rate.amount.value);
            BOOST_REQUIRE(std::abs((fill_op.deposited - fill_op.withdrawn *
                                                        gpo.get_vesting_share_price()).amount.value) <=
                          1);
            validate_database();
            ...
            for (int i = 1; i < STEEMIT_VESTING_WITHDRAW_INTERVALS - 1; i++) {
                ...
                if (i == STEEMIT_VESTING_WITHDRAW_INTERVALS - 1)
                    ...
                    BOOST_REQUIRE(
                            ...
                            (old_next_vesting +
                             STEEMIT_VESTING_WITHDRAW_INTERVAL_SECONDS).sec_since_epoch());
            ...
            if (to_withdraw.amount.value % withdraw_rate.amount.value != 0) {
                ...
                generate_blocks(db.head_block_time() +
                                STEEMIT_VESTING_WITHDRAW_INTERVAL_SECONDS, true);
                fill_op = get_last_operations(1)[0].get<fill_vesting_withdraw_operation>();
                gpo = db.get_dynamic_global_properties();
        ...
        try {
...

            {
                ...
                BOOST_REQUIRE(alice.vesting_shares ==
                              old_alice_vesting - vesting_withdraw_rate);
                BOOST_REQUIRE(alice.balance == old_alice_balance +
                                               asset((vesting_withdraw_rate.amount *
                                                      STEEMIT_1_PERCENT * 20) /
                                                     STEEMIT_100_PERCENT, VESTS_SYMBOL) *
                                               db.get_dynamic_global_properties().get_vesting_share_price());
                BOOST_REQUIRE(bob.vesting_shares == old_bob_vesting +
                                                    asset((vesting_withdraw_rate.amount *
                                                           STEEMIT_1_PERCENT *
                                                           50) /
                                                          STEEMIT_100_PERCENT, VESTS_SYMBOL));
                ...
                BOOST_REQUIRE(sam.balance == old_sam_balance +
                                             asset((vesting_withdraw_rate.amount *
                                                    STEEMIT_1_PERCENT * 30) /
                                                   STEEMIT_100_PERCENT, VESTS_SYMBOL) *
                                             db.get_dynamic_global_properties().get_vesting_share_price());
    ...
    BOOST_AUTO_TEST_CASE(sbd_interest) {
        try {
            ...
            generate_blocks(db.head_block_time() + fc::seconds(
                    STEEMIT_SBD_INTEREST_COMPOUND_INTERVAL_SEC / 2), true);
            ...
            auto alice_coindays = uint128_t(alice_sbd.amount.value) *
                                  (db.head_block_time() -
                                   start_time).to_seconds();
            alice_sbd = db.get_account("alice").sbd_balance;
            start_time = db.get_account("alice").sbd_seconds_last_update;
            ...
            generate_blocks(db.head_block_time() + fc::seconds(
                    (STEEMIT_SBD_INTEREST_COMPOUND_INTERVAL_SEC * 7) /
                    3), true);
    ...
    BOOST_AUTO_TEST_CASE(liquidity_rewards) {
        ...
        try {
            ...
            alice_steem_volume += (asset(alice_sbd.amount / 20, SBD_SYMBOL) *
                                   exchange_rate).amount.value;
            alice_reward_last_update = db.head_block_time();
            bob_steem_volume -= (asset(alice_sbd.amount / 20, SBD_SYMBOL) *
                                 exchange_rate).amount.value;
      ...
      BOOST_CHECK( reward->last_update == bob_reward_last_update );*/
            ...
            BOOST_CHECK(limit_order_idx.find(std::make_tuple("alice", 1)) ==
                        limit_order_idx.end());
            BOOST_CHECK(limit_order_idx.find(std::make_tuple("bob", 2)) ==
                        limit_order_idx.end());
            ...
            op.amount_to_sell = asset((alice_sbd.amount.value / 10) * 3 -
                                      alice_sbd.amount.value /
                                      20, STEEM_SYMBOL);
            op.min_to_receive = asset((alice_sbd.amount.value / 10) * 3 -
                                      alice_sbd.amount.value / 20, SBD_SYMBOL);
            ...
            op.min_to_receive = asset(
                    (alice_sbd.amount.value / 10) * 3, STEEM_SYMBOL);
            ...
            bob_sbd_volume += (alice_sbd.amount.value / 10) * 3 -
      ...
      BOOST_CHECK( reward->last_update == sam_reward_last_update );*/
            ...
            op.min_to_receive = asset(
                    alice_sbd.amount.value / 20 * 2, STEEM_SYMBOL);
            ...
            generate_blocks(db.head_block_time() + fc::seconds(
                    STEEMIT_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10.to_seconds() /
                    2), true);
      ...
      BOOST_CHECK( reward->last_update == dave_reward_last_update );*/
            ...
            generate_blocks(
                    ...
                    fc::seconds(STEEMIT_BLOCK_INTERVAL / 2) -
            ...
            generate_blocks(
                    db.head_block_time() + (STEEMIT_BLOCK_INTERVAL / 2) +
                    STEEMIT_LIQUIDITY_TIMEOUT_SEC, true);
    ...
    BOOST_AUTO_TEST_CASE(post_rate_limit) {
        try {
            ...
            BOOST_REQUIRE(
                    db.get_comment("alice", string("test3")).reward_weight ==
                    STEEMIT_100_PERCENT);
            ...
            BOOST_REQUIRE(
                    db.get_comment("alice", string("test4")).reward_weight ==
                    STEEMIT_100_PERCENT);
            ...
            BOOST_REQUIRE(
                    db.get_comment("alice", string("test5")).reward_weight ==
                    STEEMIT_100_PERCENT);
        }
        FC_LOG_AND_RETHROW()
    }
    ...
    BOOST_AUTO_TEST_CASE(comment_freeze) {
        try {
            ...
            BOOST_REQUIRE(
                    ...
                    0);
            BOOST_REQUIRE(
                    db.get_comment("alice", string("test")).abs_rshares.value ==
                    0);
   ...
   FC_LOG_AND_RETHROW()
    ...
    BOOST_AUTO_TEST_CASE(sbd_price_feed_limit) {
        try {
            ...
            auto new_exchange_rate = price(gpo.current_sbd_supply, asset(
    ...
    BOOST_AUTO_TEST_CASE(clear_null_account) {
        try {
            ...
            BOOST_REQUIRE(
                    ...
                    ASSET("5.000 TBD"));
            BOOST_REQUIRE(
                    db.get_account("alice").balance == ASSET("2.000 TESTS"));
            BOOST_REQUIRE(
                    db.get_account("alice").sbd_balance == ASSET("3.000 TBD"));
